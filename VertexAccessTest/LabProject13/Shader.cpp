...
{
	m_pCommandList = pd3dCommandList;
	...
	*pd3dCommandList)
{
	...
	if (pressed)
	{
		CMesh *test = m_ppObjects[0]->GetMesh();
		CDiffusedVertex *fakeVertex = new CDiffusedVertex[test->m_nVertices];
		memcpy(fakeVertex, test->m_pVertices, test->m_nStride * test->m_nVertices);

		XMFLOAT4X4 rotation{
			0.15425144988f, 0, -0.98803162409f, 0,
			0, 1, 0, 0,
			0.98803162409f, 0, 0.15425144988f, 0,
			0, 0, 0, 1
		};

		for (int i = 0; i < test->m_nVertices; ++i)
		{
			fakeVertex[i].m_xmf3Position = Vector3::TransformCoord(fakeVertex[i].m_xmf3Position, rotation);
		}
