...
////////////////////////////////////////////////////////////////////////
// 공개 함수
bool CFramework::Initialize(HINSTANCE hInstance, HWND hWnd, Network network)
{
	m_hWnd = hWnd;
	m_Network = network;
	...
	WSAAsyncSelect(m_Network.m_mysocket, hWnd, WM_SOCKET, FD_CLOSE | FD_READ);
	...
	//m_Network.Initialize(hWnd); //이니셜라이즈만 함수로 안하면 문제없음
	...
	WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
	{
		...
		if (0 != y) {
			...
			WSASend(m_Network.m_mysocket, &m_Network.m_send_wsabuf, 1, &iobyte, 0, NULL, NULL);
		}
		m_Network.ReadPacket(m_Network.m_mysocket);*/
		//m_Network.SendMovePacket(wParam);
		break;
	}
	case WM_SOCKET:
		{
...
void CFramework::BuildObjects()
{
	...
	m_pScene = new CScene();
	m_pScene->Initialize(&m_createMgr,m_Network);
