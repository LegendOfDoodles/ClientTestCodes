...
void CSoundManager::loading()
{
	...
	ErrorCheck(m_Result);
	m_Result = m_pFmod->createSound("Sound/jaguar.wav", FMOD_DEFAULT, NULL, &m_ppMusic[SOUND::SOUND_1]);
	ErrorCheck(m_Result);
	m_Result = m_pFmod->createSound("Sound/wave.mp3", FMOD_DEFAULT, NULL, &m_ppMusic[SOUND::SOUND_2]);
	ErrorCheck(m_Result);
}
